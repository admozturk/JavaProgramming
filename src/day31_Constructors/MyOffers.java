...
public class MyOffers {
    public static void main(String[] args) {
        ...
        offer4.setInfo("TX","CapitalOne", "SM", 115000, false,false,false,true);
        ...
        fullTimeOffers.removeIf(p -> !p.isFullTime); // removes if the offer is not full-time job
        System.out.println(fullTimeOffers.size());


        ArrayList<Offer> localOffers = new ArrayList<>(Arrays.asList(myoffers));
        localOffers.removeIf(p -> !p.location.equals("VA")); // removes offer if the offer is not from local area
